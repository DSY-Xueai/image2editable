...
- challenge metadata 仅保存 Schema、图片路径、PNG 哈希和 challenge ID，不保存答案、nonce 或可复现布局的随机状态。形状、颜色和数量只在生成时从安全随机源选择；验证端先用 Task 4 Run integrity key 认证 PNG 哈希/ID，再独立解析已绑定 PNG 像素得到答案。即使同权限 Agent 读取 metadata 与 key，也不能脱离视觉图像推导答案；已有 challenge 时 key 缺失、损坏或被替换均 fail closed。PNG 与 metadata 在唯一 staging 目录内完整验证后整目录发布，写入/rename 故障清理自有 staging 后可重试，并发 next 复用同一完整发布。
